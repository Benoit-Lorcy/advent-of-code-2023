let input = `.7--77|.|-|77F|.J.|.FF.J-|7F--7F7.J----7FJ77J..F77-77-F.F|7-|F-.|F777FF7|-.-|-7|..FFL---L7-7FL7L-|-77.FL-7FF-7FJF|-7|7-FF|-|.J-FLJ7.F7.F7--L
...
for(let i = 0; i< input.length; i++){
    for(let j = 0; j< input[i].length; j++){
        if(input[i][j] === "S") s = {y:i, x:j};
    }
}

let adj = [{y:0, x:1}, {y:1, x:0}, {y:-1, x:0}, {y:0, x:-1}]
let results = []

for(el of adj){
    if(s.y + el.y < 0 || s.x + el.x < 0) continue;
    let curr = {y: s.y + el.y, x:s.x + el.x};
    let last = el;
    let path = [];
    path.push(curr)
    let res = 0;
    while(input[curr.y][curr.x] !== "S"){
        let temp = input[curr.y][curr.x];
        last = pipeline(temp, last);
        if(last === null){
            console.log("rees")
            break;
        }
        curr = {x: curr.x +last.x, y:curr.y + last.y}
        path.push(curr)
        res++;
    }
    results.push({res:res, path:path})
}

let res2 = {};
for(let i = 0; i < 4; i++){
    for(let j = 0; j < 4; j++){
        if(i !== j && results[i].res === results[j].res && results[i].res !== 0){
            res2 = results[i]
        }
    }
}

res2.path = [res2.path[res2.path.length-1], ...res2.path]
console.log(res2.path)
let air = 0;
for(let i = 0; i < res2.path.length - 1; i++){
    air += (res2.path[i].x + res2.path[i+1].x) * (res2.path[i].y - res2.path[i+1].y)
}
console.log(-air / 2 - res2.res /2 + 1)

function pipeline(pipe, last){
    if(pipe === ".") return null;
    if(pipe === "|" || pipe === "-") return last;

    if(last.x !== 0){
        if(pipe === "L" || pipe === "J") return {y:-1, x:0}
        if(pipe === "7" || pipe === "F") return {y:1, x:0}
    } else {
        if(pipe === "L" || pipe === "F") return {y:0, x:1}
        if(pipe === "J" || pipe === "7") return {y:0, x:-1}
    }
}