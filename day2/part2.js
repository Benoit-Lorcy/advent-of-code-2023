let input = `Game 1: 2 green, 12 blue; 6 red, 6 blue; 8 blue, 5 green, 5 red; 5 green, 13 blue; 3 green, 7 red, 10 blue; 13 blue, 8 red
...
for(let i = 0; i< input.length; i++){
    let red = Math.max(...input[i].match(/(\d*) red/gm).map(e => +e.split(" ")[0]))
    let blue = Math.max(...input[i].match(/(\d*) blue/gm).map(e => +e.split(" ")[0]))
    let green = Math.max(...input[i].match(/(\d*) green/gm).map(e => +e.split(" ")[0]))
    res += red * blue * green
}
