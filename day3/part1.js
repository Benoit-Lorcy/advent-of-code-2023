...
for(let i = 0; i < input.length; i++){
    for(let j = 0; j < input[i].length; j++){
        if(input[i][j] && input[i][j].match(/\.|\d/gm) == null){
            res += getAjdNumber(input, i, j)
        }
    }
}

function getAjdNumber(input, i, j){
    let partList = []

    for(let k = i - 1; k < i + 2; k++){
        for(let m = j - 1; m < j + 2; m++){
            //console.log(k,m)
            if(input[k][m] && input[k][m].match(/\d/gm)){
                let part = []
                let t = 1
                part.push(input[k][m])
                input[k][m-0] = "."
                while(input[k][m-t] && input[k][m-t].match(/\d/gm)){
                    part.splice(0,0,input[k][m-t])
                    input[k][m-t] = "."
                    t++;
                }
                t = 1
                while(input[k][m+t] && input[k][m+t].match(/\d/gm)){
                    part.push(input[k][m+t])
                    input[k][m+t] = "."
                    t++;
                }
                partList.push(part.join(""));
